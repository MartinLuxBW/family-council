...
const api = {
  'POST /api/auth/signup': async (req, res) => {
    ...
    const r = db
      .prepare('INSERT INTO users (email, password_hash, family_name) VALUES (?, ?, ?)')
  ...
  'GET /api/week': async (req, res, user, url) => {
    ...
    // Weekly tasks automatically land on this week's pile the first time the week is opened.
    ...
    const instances = db
      .prepare(`${INSTANCE_SELECT} WHERE i.user_id = ? AND i.week_start = ? ORDER BY i.id`)
  ...
  'POST /api/members': async (req, res, user) => {
    ...
    const r = db
      .prepare('INSERT INTO members (user_id, name, role, color) VALUES (?, ?, ?, ?)')
  ...
  'POST /api/members/:id/image': async (req, res, user, url, m) => {
    ...
    // The upload becomes both the stored original (for cartoonify) and the displayed avatar.
  ...
  'POST /api/members/:id/generate': async (req, res, user, url, m) => {
    ...
    if (mode === 'photo' && !member.photo) {
      return json(res, 400, { error: 'Upload a photo first, then I can cartoonify it!' });
    }
    try {
  ...
  'POST /api/tasks': async (req, res, user) => {
    ...
    const r = db
      .prepare('INSERT INTO tasks (user_id, title, emoji, type) VALUES (?, ?, ?, ?)')
      .run(user.id, title.trim(), emoji || '⭐', type);
    // Drop it onto the pile of the week currently being planned.
    if (/^\d{4}-\d{2}-\d{2}$/.test(weekStart ?? '')) {
      db.prepare('INSERT INTO instances (user_id, task_id, week_start) VALUES (?, ?, ?)').run(
        user.id,
        r.lastInsertRowid,
        weekStart
      );
  ...
  'PATCH /api/instances/:id': async (req, res, user, url, m) => {
    ...
    // Assigning to a member clears the family flag and vice versa.
    if ('member_id' in body && body.member_id != null) is_family = 0;
    if ('is_family' in body && body.is_family) member_id = null;
    // A task can only be scheduled on a day once someone owns it.
    if (day != null && member_id == null && !is_family) {
      return json(res, 400, { error: 'Assign this task to someone first!' });
    }
    if (day != null && (day < 0 || day > 6)) return json(res, 400, { error: 'Invalid day' });

    db.prepare('UPDATE instances SET member_id = ?, is_family = ?, day = ?, completed = ? WHERE id = ?').run(
      member_id,
      is_family ? 1 : 0,
      day,
      completed ? 1 : 0,
  ...
  'DELETE /api/instances/:id': async (req, res, user, url, m) => {
    ...
    // One-time tasks live and die with their single instance.
  ...
  'GET /api/images/:name': async (req, res, user, url, m) => {
    const name = path.basename(decodeURIComponent(m.name)); // strips any path traversal
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const file = path.join(PUBLIC, path.normalize(url.pathname).replace(/^([.][.][/\\])+/, ''));
